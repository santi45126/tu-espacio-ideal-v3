...
const authToggleLink = document('auth-toggle-link');
const authTitle = document.getElementById('auth-title');
const authNameGroup = document.getElementById('auth-name-group');
const logoutButton = document.getElementById('logout-button');
const userNameDisplay = document.getElementById('user-name-display'); // Nuevo elemento para mostrar el nombre del usuario
const userEmailDisplay = document.getElementById('user-email-display'); // Nuevo elemento para mostrar el email del usuario
...
let currentUser = null; // Firebase manejará el estado de autenticación
...
imageUpload.addEventListener('change', function() {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            imageUrlUnchanged.value = 'false'; // Indica que se subió una nueva imagen
        };
...
async function loadDepartments(page = 1, userOnly = false) {
    currentPage = page;
    try {
        const departmentsRef = window.collection(window.db, 'departamentos'); // Referencia a la colección 'departamentos'
        let q;

        if (userOnly && currentUser && currentUser.uid) {
            // Si se piden solo los del usuario, filtramos por userId
            q = window.query(departmentsRef, window.where('userId', '==', currentUser.uid));
        } else {
            // Para todos los departamentos, se aplica la paginación con query limits
            // Nota: La paginación real con Firestore requiere `orderBy` y `startAt`/`startAfter` para ser eficiente.
            // Para simplificar por ahora, obtendremos todos y filtraremos en JS si es necesario,
            // pero para apps grandes, se necesitará una paginación más avanzada en Firestore.
            q = departmentsRef; // Obtener todos para el filtro actual, si no hay userOnly
        }
        ...
        // Simulación de paginación y conteo total (Firestore no devuelve x-total-count directamente)
        totalDepartmentsCount = fetchedDepartments.length; // Contamos todos los que trajimos
        allDepartments = fetchedDepartments;

        filterAndSortDepartments(userOnly); // Filtramos y ordenamos la data ya cargada
        // No necesitamos updateUserUI aquí, onAuthStateChanged ya lo maneja al inicio.
...
function displayDepartments(departmentsToDisplay) {
    ...
    departmentsToDisplay.forEach(dep => {
        ...
        card.innerHTML = `
            ...
            ${currentUser && dep.userId === currentUser.uid ? // Usamos currentUser.uid para comparar con el userId del departamento
...
function showDepartmentDetails(id) {
    ...
    if (department) {
        ...
        // Asegúrate de que este elemento exista en tu HTML si lo usas
...
async function saveDepartment(departmentData, imageFile) {
    ...
    departmentData.userId = currentUser.uid; // Asignamos el UID de Firebase al departamento
    ...
    try {
        if (imageFile) {
            ...
            // Si es edición y no se cambió la imagen, mantener la URL existente
            ...
            departmentData.image_url = null; // No hay imagen
        }
        ...
        closeModal(modalPublish);
        // Recargar departamentos, mostrando los del usuario si estaba en esa vista
...
async function deleteDepartment(id) {
    ...
    try {
        ...
        // Opcional: Verificar que el usuario sea el propietario antes de eliminar (regla de seguridad de Firestore ya lo maneja, pero es bueno tenerlo también en el cliente)
...
async function editDepartment(id) {
    ...
    if (department) {
        ...
        imageUrlUnchanged.value = 'true'; // Se asume que la URL de imagen existente es válida
...
function filterAndSortDepartments(userOnly = false) {
    ...
    let filteredDepartments = allDepartments.filter(dep => {
        ...
        // Ya cargamos userOnly desde Firestore, así que no es necesario filtrar de nuevo aquí
        // a menos que quieras hacer un filtro adicional en cliente.
        // La función loadDepartments ya se encarga de traer solo los del usuario si userOnly es true.
        // Pero si estamos mostrando "todas las publicaciones" y luego el usuario activa "mis publicaciones"
        // sin recargar, esto filtraría sobre lo ya cargado.
...
sortBySelect.addEventListener('change', () => {
    // Cuando cambia el orden, no necesitamos recargar de Firestore, solo reordenar lo que ya tenemos
...
publishForm.addEventListener('submit', async e => {
    ...
    const departmentData = {
        ...
        // image_url se maneja en saveDepartment
...
contactForm.addEventListener('submit', async (e) => {
    ...
    console.log('Consulta enviada:', {
        departmentTitle: document.getElementById('department-title').textContent, // Usar el título del departamento como referencia
        inquirerName,
        inquirerEmail,
        inquirerPhone,
        inquirerMessage
...
function renderPaginationControls() {
    // Con Firestore, la paginación a menudo se maneja en el query (limit y startAfter/startAt).
    // Aquí, estamos simulando una paginación básica sobre los resultados filtrados.
    // Para grandes volúmenes de datos, necesitarías ajustar loadDepartments con queries más avanzados de Firestore.
...
function goToPage(page) {
    ...
    // Por ahora, solo recargamos la página entera con el filtro.
    // Para paginación real de Firestore, se necesitaría pasar el último documento de la página anterior.
    currentPage = page;
    loadDepartments(currentPage, userOnly);
}

// --- Autenticación con Firebase Auth ---
// `onAuthStateChanged` es el observador clave para el estado del usuario
window.onAuthStateChanged(window.auth, (user) => {
    currentUser = user; // `user` será null si no hay sesión, o el objeto de usuario de Firebase si hay sesión.
    updateUserUI(); // Actualiza la UI basada en el estado del usuario.
    // Recarga los departamentos para reflejar permisos o filtros de "mis publicaciones"
    loadDepartments(currentPage, userActionButton.dataset.view === 'my-listings');
});
...
function updateUserUI() {
    if (currentUser) {
        ...
        userNameDisplay.textContent = currentUser.displayName || ''; // Muestra el nombre
        userEmailDisplay.textContent = currentUser.email || ''; // Muestra el email
    ...
    // Asegurarse de que si se desloguea, la vista vuelva a "todas las publicaciones"
    if (!currentUser && userActionButton.dataset.view === 'my-listings') {
        userActionButton.dataset.view = 'all'; // Cambia el estado del botón
        // No es necesario llamar a loadDepartments aquí porque onAuthStateChanged ya lo hace.
    }
    // Si currentUser es null, y userActionButton está en 'my-listings', la loadDepartments de onAuthStateChanged
    // ya lo va a corregir a 'all' y recargará.
}

userActionButton.addEventListener('click', () => {
    if (currentUser) {
        // Toggle entre "Mis Publicaciones" y "Todas las Publicaciones"
        if (userActionButton.dataset.view === 'all') {
            ...
            loadDepartments(1, true); // Cargar solo las del usuario
        } else {
            userActionButton.dataset.view = 'all';
            userActionButton.textContent = `Hola, ${currentUser.displayName || currentUser.email} (Mis Publicaciones)`;
            loadDepartments(1, false); // Cargar todas las publicaciones
        }
    } else {
        // Si no hay sesión, abrir modal de autenticación
        authTitle.textContent = 'Inicia Sesión o Regístrate';
        authNameGroup.style.display = 'block'; // Mostrar campo de nombre por defecto para registro
...
authForm.addEventListener('submit', async (e) => {
    ...
    const password = document.getElementById('auth-password').value.trim(); // Nuevo campo de contraseña
    ...
    try {
        if (mode === 'register') {
            ...
            // CAMBIO CLAVE AQUÍ: Llama a window.updateProfile pasando el objeto user
        ...
        closeModal(modalAuth);
        // onAuthStateChanged se encargará de actualizar la UI y cargar los departamentos.
    } catch (error) {
...
authToggleLink.addEventListener('click', (e) => {
    ...
    if (authForm.dataset.mode === 'register') {
        ...
        authNameGroup.style.display = 'none'; // Ocultar campo de nombre para login
        authToggleLink.textContent = '¿No tienes una cuenta? Regístrate aquí.';
    } else {
        authForm.dataset.mode = 'register';
        authTitle.textContent = 'Registrarse';
        authNameGroup.style.display = 'block'; // Mostrar campo de nombre para registro
        authToggleLink.textContent = '¿Ya tienes una cuenta? Inicia Sesión aquí.';
    }
    authForm.reset(); // Limpiar el formulario
});
...
// No necesitamos un DOMContentLoaded listener explícito si todo se carga por módulos y eventos.
// La llamada inicial a loadDepartments se hace desde onAuthStateChanged.