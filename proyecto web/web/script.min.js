...
// --- INICIO DE CÓDIGO DE VALIDACIÓN AÑADIDO ---
    ...
    // Validar Habitaciones: debe ser un número entero positivo
    if (isNaN(o.bedrooms) || o.bedrooms <= 0 || !Number.isInteger(o.bedrooms)) {
        showToast('Las habitaciones deben ser un número entero positivo.', 'error');
        return;
    }

    // Validar Baños: debe ser un número positivo (permite decimales como 0.5)
    if (isNaN(o.bathrooms) || o.bathrooms <= 0) {
        showToast('Los baños deben ser un número positivo.', 'error');
        return;
    }

    // Validar Contacto: un formato básico de teléfono o email
    const contactRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$|^[0-9\s\-\(\)\+]{7,}$/; // Email o al menos 7 dígitos para teléfono
    if (!contactRegex.test(o.contact)) {
        showToast('Formato de contacto inválido. Ingrese un email o un número de teléfono válido (mín. 7 dígitos).', 'error');
        return;
    }
// --- FIN DE CÓDIGO DE VALIDACIÓN AÑADIDO ---

if(imageUpload.files.length>0&&'false'===imageUrlUnchanged.value){o.image_url='https://picsum.photos/400/300?'+Math.random()}else if(editingDepartmentId&&'true'===imageUrlUnchanged.value){const e=allDepartments.find(e=>e.id===editingDepartmentId);o.image_url=e?e.image_url:null}else{o.image_url=null}await saveDepartment(o)});document.addEventListener('DOMContentLoaded',loadDepartments);